...
<?php 
			...
			<div class="table-responsive no-scroll">
				...
				while($compLc = mysql_fetch_array($result_view_locc))
				{
	...
	foreach ($locationc as $value) {
		?>
             <h3 style="margin-bottom:10px;"><?php echo $value; ?></h3><?php
			  $sql_data = mysql_query("select * from personal_details where Notification='$rw' AND post_applied='$value' AND eligibilty='E'");
					$a=1;
			 ...
			 ?>
			   ...
			   <?php
                   ...
                   while($result_data = mysql_fetch_array($sql_data)){
					   $id=$result_data['id'];
					   ...
					   $query=mysql_query($query_per);
					  $result_data1 = mysql_fetch_array($query);
					  $marks=$result_data1['marks'];
					  //====================medium===============================
					  	$query_pe= "SELECT DISTINCT (medium_of_insruction) as 'medium' FROM acadmic_detail where id='$id'";
						  $query11=mysql_query($query_pe);
					  $result_data11 = mysql_fetch_array($query11);
