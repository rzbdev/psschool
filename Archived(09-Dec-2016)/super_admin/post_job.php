...
if($post=='post1')
{
	 $sql=mysqli_query("UPDATE manage_email SET email_send='$status' WHERE id=$user_id");
	
	 $select=mysqli_query("select name from manage_email  WHERE id=$user_id");
	   $row12=mysqli_fetch_array($select);
      $name=$row12['name'];
...
if($post=='post')
{
 $sql=mysqli_query("UPDATE manage_email SET post='$status' WHERE id=$user_id");
	...
	if($post=='post2')
{
	echo"UPDATE manage_email SET send_email='$status'";
		 $sql=mysqli_query("UPDATE manage_email SET send_email='$status'");
	} 
