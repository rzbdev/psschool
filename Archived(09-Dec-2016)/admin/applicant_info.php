...
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css">
	...
	<link href='
	https://fonts.googleapis.com/css?family=Source+Sans+Pro:400,200,200italic,300,400italic,600,300italic,600italic,700,700italic,900,900italic|Playfair+Display:400,400italic,700,700italic,900,900italic' rel='stylesheet' type='text/css'>
...
<style>
.view-table th > strong { font-size: 13px;text-transform:uppercase;line-height: 18px;}
.view-table th  { font-size: 12px;text-transform:uppercase;line-height: 18px;}
.view-table td > strong { font-size: 13px;text-transform:uppercase;line-height: 18px;}
.print-btn2 {position: absolute;right: 90px;z-index: 999;top: 21px;}
.print-btn1 {position: absolute;right: 153px;z-index: 999;top: 21px;}
.print-btn3 {position: absolute;right: 210px;z-index: 999;top: 21px;}
.print-btn4 {position: absolute;right: 30px;z-index: 999;top: 21px;}
</style>
	<?php   
...
if(!isset($_SESSION["alid"]))
{
	...
	echo 'window.location.href ="login.php?id='.$id.'"';
	echo '</script>';
} 
	?>
	  <?php 
...
<div class="container-fluid">
		<div class="col-md-12">
		<a href="htmlToPdf/actionpdf.php?id=<?php echo $id; ?>" class="btn btn-primary print-btn3" target="_blank">Pdf1</a>
		<a href="Generate_pdf/actionpdf.php?id=<?php echo $id; ?>" class="btn btn-primary print-btn1" target="_blank">Pdf2</a>
		<a href="download2.php?id=<?php echo $id; ?>" class="btn btn-primary print-btn2">Excel</a>
		 <a onclick="goBack()" class="btn btn-primary print-btn4" data-dismiss="modal" >Back</a>
		<div class="table-responsive">
				<table class="view-table">
				  <tbody>
				  <tr class="hdng">
				  <td colspan="3" style="padding:12px 15px;">Basic Information</td>
				  </tr>
...
<td><?php echo $ctet_tet_detail;  ?> </td>					  
          ...
          echo date('d/m/Y', $timestamp);
				...
				<div class="col-md-12 log_maintain">
					...
					<?php
					 $rep1="select id from personal_details where (first_name='$first_name' AND last_name='$Last_Name' AND dob='$dob' ) AND Notification='$rw' AND ((father_name='' AND husband='$husband') OR (father_name='$FATHER' AND husband='')) ORDER BY id DESC LIMIT 1,1"; 
							$row_rep1=mysqli_query($rep1);
					if($log_row11=mysqli_fetch_array($row_rep1))
					{
						$rep_applicant_id=$log_row11['id'];
				 $rep_rec="select * from eligible where ulid=$rep_applicant_id";		$row_rec=mysqli_query($rep_rec);
				$rep_log_row11=mysqli_fetch_array($row_rec);
				...
				$row_rec1=mysqli_query($rep_rec1);
				$rep_log_row12=mysqli_fetch_array($row_rec1);
				 $rep_admin=$rep_log_row12['name'];
					}
				$rep="select * from personal_details where id=$id";
				$row_rep=mysqli_query($rep);
...
else if($rep_eligibility=='NE')
{
 ...
 while($log_row1=mysqli_fetch_array($row_rep))
	 
 {
	...
	<?php
	 ...
	 else
	 {
					...
					<tr><th>
			<strong style="font-size:14px;font-family:Helvetica,Arial,sans-serif;">COMMENTS</strong>
			</th></tr>
			
				<?php
				 $log_sqll1="select * from log_eligible_table where user_id=$id";
 $log_result=mysqli_query($log_sqll1);
$nnn=mysqli_num_rows($log_result);
...
echo " ".$admin_name." changed the status of applicant ".$status.".<br>Comment: ".ucfirst(strtolower($comment)).". Date:".date('d/m/Y', $timestamp)."  <br><br> </td></tr>";
 ...
 $log_result1=mysqli_query($log_sqll11);
 
 while($log_row1=mysqli_fetch_array($log_result1))
 {
	 echo "<tr><td>";
...
<div class="col-md-12 row">					  
				...
				<div class="col-md-12">
				  ...
				  <div class="row">
				  <div class="col-md-12">
				  <a onclick="goBack()" class="btn btn-close pull-right" data-dismiss="modal" >Back</a>
...
<?php $sss="select eligibilty from personal_details where id='$id'";
  $sss1=mysqli_query($sss);
  $sss11=mysqli_fetch_array($sss1);
  $ee=$sss11['eligibilty'] ;?>
...
setTimeout(function() { $("#testdiv").fadeOut(1500); }, 5000)
	...
	$(".eligible").on("click",function(){
		...
		$("#status_hidden").val(result);
				...
				<div class="clearfix"></div>
				<br/><br/><br/>
<div class="ff" style="bottom:inherit;">
				<div class="container-fluid">
