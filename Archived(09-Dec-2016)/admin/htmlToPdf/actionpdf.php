...
$id = $_REQUEST['id'];
				...
				$query="select * from  personal_details where id=$id";
				  ...
				  while($row=mysqli_fetch_array($row1))
				  {
					 ...
					 $dob1111=$row['dob'];
					  $dob11 = explode("/",$dob1);
								   $dob111=$dob11[0].'-'.$dob11[1].'-'.$dob11[2];
					 $age=$row['age'];
				...
				$address=$row['address'];
					 ...
					 if($comm_address1!='')
					  {
					   $communication_address=$row['communication_address'].'<br>';
					  }
					  else{
					 ...
					 $per_mobile1=$row['contact_no'];
					  ...
					  if($per_address1!='')
					  {
					  $PERMANENT_ADDRESS=$row['address'].'<br>';
					  }
					  else{
...
if($check2==(mysqli_num_rows($result6)-1))
		{
					  ...
					  if($check3==0)
{
	...
	$nontechexp1="";
		...
		if($check3==(mysqli_num_rows($result7)-1))
		{
...
if($check4==(mysqli_num_rows($row1)-1))
		{
...
$pdf->AliasNbPages();
$pdf->SetMargins(20,1,1);
$pdf->SetAutoPageBreak(true, 15);

$pdf->AddPage();
$pdf->Image('http://'.$server.'/uploads/'.$image1.'',174,13,16);
...
<TD>'.$PERMANENT_ADDRESS.''.$per_address1.'<br>City :&nbsp;'.$city.',&nbsp;Pincode :&nbsp;'.$pin.'<br>State :&nbsp;'.$state.'<br>Telephone :&nbsp;'.$tele_no.'<br>Mobile No. :&nbsp;'.$per_mobile1.'</TD>
</TR>
<TR>
<td><strong>ADDRESS FOR COMMUNICATION</strong></td>
<TD>'.$communication_address.''.$comm_address1.'<br>City :&nbsp;'.$comm_city.',&nbsp;Pincode :&nbsp;'.$comm_pin.'<br>State :&nbsp;'.$comm_state.'<br>Telephone :&nbsp;'.$comm_telephoneno.'</TD>					 
</TR>
<TR>
...
<TR>
<td><strong>  N.C.C. or N.S.S. or NATIONAL SCOUT TRAINING, IF ANY? </strong></td>					 
<TD>'.$ncc_detail.'</TD>
</TR>
<TR>
...
<TR>
<td><strong>ARE YOU TECHNO-SAVVY or COMPUTER LITERATE?</strong></td>					 
<TD>'.$techosavvy_computerliterate_detail.'</TD>
</TR>
<TR>
<td><strong>LITERARY, CULTURAL, SPORTS OR OTHER ACTIVITIES IN WHICH THE APPLICANT HAS DISTINCTIONS or AWARDS</strong></td>					 
<TD>'.$awards_detail.'</TD>
</TR>
<TR>
...
<TR>
<td><strong>  ARE YOU CTET or TET QUALIFIED? </strong></td>					 
...
<TD>'.$f_name_ref.'<br>'.$l_name_ref.'<br>'.$address_ref.''.$address_reff.'<br>'.$ref_city.',&nbsp;'.$pin_ref.'<br>'.$ref_state.'<br>'.$telephone_ref.'<br>'.$mobile_ref.'<br>'.$email_ref.'<br>'.$designation_ref.'<br></td>
</TR>
<tr>
<td></td>
<TD>'.$f_name1_ref.'<br>'.$l_name1_ref.'<br>'.$address1_ref.''.$address1_reff.'<br>'.$ref_city1.',&nbsp;'.$pin_ref1.'<br>'.$ref_state1.'<br>'.$telephone1_ref.'<br>'.$mobile1_ref.'<br>'.$email_ref1.'<br>'.$designation1_ref.'<br></td>
<tr>
</tbody>
</TABLE>';
