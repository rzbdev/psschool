...
<div class="container-fluid">
 ...
 <?php 
	...
	{
		$selectnn="SELECT *  FROM notification where date_ex < '$current_date' order by date_ex desc";
			$rownn=mysql_Query($selectnn);
		$ro=mysql_fetch_array($rownn);
		...
		<!---<button type="submit" class="btn srch" name="search">Search</button>-->
		 ...
		 <div class="choose-numbr">
			...
			<select class="form-control" onchange="ajaxFunction('');" name="" id="job" placeholder="Quick Search">
			 ...
			 <?php 
				 $selec="select * from jobs where nid IN(SELECT nid FROM notification where status='1')";
	        	$r=mysql_Query($selec);
	...
	while($rowt=mysql_fetch_array($r))
	{
		$title=$rowt['title'];
	$post = htmlspecialchars($_GET['post']);	
if(isset($post) && $post==$title) {
		?>
	<option value="<?php echo $title; ?>" selected><?php echo $title; ?></option> <?php } else {?>
	  <option value="<?php echo $title; ?>"><?php echo $title; ?></option> 		
	<?php 		
	}} }
	else
	{
		$s="SELECT nid FROM notification where date_ex < '$current_date' order by date_ex desc";
		$r1=mysql_Query($s);	
		$rowmm=mysql_fetch_array($r1);
		$nid_new= $rowmm['nid'];
	$selec="select * from jobs where nid='$nid_new'";
	        	$r=mysql_Query($selec);				
...
if(isset($post) && $post==$title) {
	...
	?>
		...
		<?php 	  $select1="SELECT MAX(nid) FROM notification";
	...
	$ro=mysql_fetch_array($row1)
		...
		?>
		<?php if(isset($_GET['e'])) { ?>
			  <option class="hello" value="E" selected>Eligible</option><?php } else { ?>
			  			    <option class="hello" value="E" >Eligible</option><?php } ?>
			  
			    <?php if(isset($_GET['ne'])) { ?>
			  <option value="NE" class="b" selected >Non-Eligible</option><?php } else { ?>
			  			    <option value="NE" class="b" >Non-Eligible</option><?php } ?>
			  <?php if(isset($_GET['nr'])) { ?>

			  <option value="NR" class="c" selected>Not-Rated</option><?php } else { ?>
			  <option value="NR" class="c" >Not-Rated</option> <?php } ?>
			</select>
			</div>
		...
		<div class="col-md-3 col-sm-12" style="margin-top: 10px">
			<?php
			$select2="SELECT MAX(nid) FROM notification";
	$row2=mysql_Query($select2);
    $rc=mysql_num_rows($row2);
	$ro=mysql_fetch_array($row2);
	?>
				<a  style="float:right; href="#"  id="download" class="btn srch">Download All</a>
	
			 <!--<a style="float:right;" href="old_applicants.php?page=1"><button type="button" class="btn srch rght1">OLD APPLICANTS</button></a>-->
			...
			<div id="" class="result11">
				...
				<col width="60px">
							...
							if($rc=='0')
							{
								$sql="UPDATE personal_details SET status='0'";
								 $row=mysql_query($sql);
							...
							{
								$sql="UPDATE personal_details SET status='1'";
								 $row=mysql_query($sql);
								?>
...
if(myarray.value.startrecord =="yes"){ document.getElementById("back").style.display='inline-block';
	...
	var url="employee-grid-data.php";
